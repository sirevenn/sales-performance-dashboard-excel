...
An Excel dashboard analysing shoe sales performance — revenue, profitability, product mix, brand/category performance, geography, and payment patterns: Built with PivotTables, PivotCharts, and Power Query.

![Dashboard Preview](charts/full_dashboard.png)

**Credit**: Built by following along with [this webinar](https://www.youtube.com/live/28JRwAWL_qg), then completed independently on the raw dirty dataset.
...
- **Microsoft Excel** : PivotTables, PivotCharts, Power Query (data cleaning), calculated fields, SUMIFS, conditional formatting
...
**7. What purchasing patterns can we identify from payment method and product preferences?**
No broad, business-wide correlation between payment method and product. Two isolated exceptions stand out: Canvas skews heavily toward Bank Transfer and away from Mobile Money, and Monk Straps skews toward Cash — neither strong enough to justify a blanket payment-method strategy.
...
- Scale `Derby` deliberately (e.g. better placement/visibility) given its outsized profit margin relative to its current low volume.
...
- Diagnose the August drop before the next planning cycle, and look to replicate whatever drove June's peak.
...
- Added calculated fields: Cost Price, Selling Price, Cost, Sale (Revenue), Profit, and Profit Margin %.
...
- Profit Margin % is calculated as SUM(Profit) ÷ SUM(Sale) at each grouping level (not a simple average of row-level percentages), to avoid skewing results toward low-volume transactions.
